...
def run(communicator):
    ...
    try:
        while 1:
            choix = input("== MENU ==\n\n1. Ajouter une musique\n2. Supprimer une musique \n3. Modifier une musique \n4. Quitter\n\nEntrez votre choix : ")
            if choix == "1":
                genre = input("Genre : ")
                nom = input("Nom : ")
                auteur = input("Auteur : ")
                clock.addMusique(genre, nom, auteur)
                print("Musique ajoutée avec succès")
            if choix == "2":
                genre = input("Genre : ")
                nom = input("Nom : ")
                auteur = input("Auteur : ")
                clock.supprimerMusique(genre,nom, auteur)
                print(" La musique {auteur}-{nom}.mp3:")
            if choix == "3":
                genre = input("Genre : ")
                oldNom = input("oldname: ")
                nvNom = input("Nouveau nom: ")
                auteur = input("Auteur : ")
                clock.updateMusique(genre,oldNom, nvNom, auteur)
                print(" La musique {auteur}-{nom}.mp3:")
            time.sleep(1)
    except IOError:
        # Ignore
